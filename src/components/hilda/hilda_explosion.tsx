...
export default function hildaExplosion({ currentView, changeView }: { currentView: String; changeView: (name: String) => void; }) {
    ...
    useEffect(() => {
        ...
        if (animatinState == "suspension" && currentView == "hilda_explosion") {
    ...
    useEffect(() => {
        if (animatinState == "intro" && canvasRef.current && currentView == "hilda_explosion") {
            animation.current = new Animations(canvasRef.current, '/resources/hilda_intro_spriteSheet.png', 16);
            animation.current.loop(false, 100, () => { setAnimationState("hilda_explosion") });
